...
int main(){
	int t; l n,i,j;
	ll temp,firstHalfSum,secondHalfSum;
	ll minSum,maxSum;
	...
	while(t-->0){
		SL(n);
		minSum = 0; maxSum = 0;
		firstHalfSum = 0; secondHalfSum = 0;
		REP(i,n)
			SLL(array[i]);
		sort(array,array+n);
		FOR(i,0,n,1){
			if(i < n/2)	firstHalfSum += array[i];
			else secondHalfSum += array[i];
			if(i%2 == 0) temp=array[i];
			else{
				temp = abs(temp-array[i]);
				minSum += temp;
			}
		}
		maxSum = abs(firstHalfSum-secondHalfSum);
